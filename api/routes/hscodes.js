...
const upload = multer({
    ...
    fileFilter: (_req, file, cb) => {
        if (file.fieldname === 'classificationNote') {
            if (classificationNoteMimeTypes.has(file.mimetype))
                cb(null, true);
            else
                cb(new Error('INVALID_CLASSIFICATION_NOTE_TYPE'));
            return;
        }
        if (file.fieldname === 'productImage') {
            if (productImageMimeTypes.has(file.mimetype))
                ...
                cb(new Error('INVALID_PRODUCT_IMAGE_TYPE'));
            return;
        }
        cb(null, false);
    },
...
const excelUpload = multer({
    ...
    fileFilter: (_req, file, cb) => {
        ...
        cb(ok ? null : new Error('INVALID_EXCEL_TYPE'), ok);
    },
});
...
function maybeNullNumber(input) {
    if (input === undefined)
        return undefined;
    if (typeof input !== 'string')
        return undefined;
...
function maybeNullDecimal(input) {
    if (input === undefined)
        return undefined;
    if (input === null)
        return null;
...
function getAbsoluteUploadPath(publicPath) {
    if (!publicPath.startsWith(`${publicUploadBase}/`))
        return null;
...
function mapRow(row) {
    return {
        ...
        fullImportFee: row.full_import_fee === null || row.full_import_fee === undefined
            ? null
            : Number(row.full_import_fee),
        consumptionSpendingFee: row.consumption_spending_fee === null || row.consumption_spending_fee === undefined
            ? null
            : Number(row.consumption_spending_fee),
        taxAdvance: row.tax_advance === null || row.tax_advance === undefined
            ? null
...
router.get('/export.xlsx', asyncHandler(async (req, res) => {
    ...
    const [rows] = await db.query(`SELECT id, code, description, notes,
            unit_type,
...
router.post('/import.xlsx', requirePermission('hscodes:write'), excelUpload.single('file'), asyncHandler(async (req, res) => {
    if (req.user?.role !== 'superadmin') {
        res.status(403).json({ success: false, error: 'FORBIDDEN' });
        return;
    }
    ...
    const readAny = (obj, keys) => {
        for (const k of keys) {
            for (const actualKey of Object.keys(obj)) {
                if (normalizeHeader(actualKey) === normalizeHeader(k))
    ...
    try {
        ...
        const [existingRows] = await conn.query(`SELECT id, code FROM hscodes WHERE code IN (${inPlaceholders})`, inParams);
        const existingByCode = new Map();
        for (const er of existingRows) {
            const code = String(er.code ?? '');
            const id = String(er.id ?? '');
            if (code && id)
                existingByCode.set(code, id);
        }
        let inserted = 0;
        let updated = 0;
        for (const r of uniqueRows) {
            ...
            if (existingId) {
                await conn.query(`UPDATE hscodes
           ...
           WHERE id = :id`, {
                ...
                const id = randomUUID();
                await conn.query(`INSERT INTO hscodes (
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const params = {};
    if (q.length > 0) {
        filters.push('(code LIKE :q OR description LIKE :q OR notes LIKE :q OR unit_type LIKE :q)');
        params.q = `%${q}%`;
    }
    ...
    const [rows] = await db.query(`SELECT id, code, description, notes, full_import_fee, consumption_spending_fee, tax_advance, unit_type,
...
router.post('/', requirePermission('hscodes:write'), hscodeUploadMiddleware, asyncHandler(async (req, res) => {
    ...
    try {
        await db.query(`INSERT INTO hscodes (
          id, code, description, notes,
...
router.patch('/:id', requirePermission('hscodes:write'), hscodeUploadMiddleware, asyncHandler(async (req, res) => {
    const schema = z.object({
    ...
    const me = req.user;
    const [existingRows] = await db.query(`SELECT id, status, created_by, classification_note_path, product_image_path
       FROM hscodes
       WHERE id = :id
       LIMIT 1`, { id });
...
router.delete('/:id', requirePermission('hscodes:write'), asyncHandler(async (req, res) => {
    if (req.user?.role !== 'superadmin') {
        res.status(403).json({ success: false, error: 'FORBIDDEN' });
        return;
    }
    ...
    const [result] = await db.query('DELETE FROM hscodes WHERE id = :id', { id });
...
router.post('/:id/approve', requirePermission('hscodes:approve'), asyncHandler(async (req, res) => {
    const id = req.params.id;
    const me = req.user;
    const [result] = await db.query('UPDATE hscodes SET status = :status, audited_by = :audited_by, audited_at = NOW(), rejection_reason = NULL, updated_by = :updated_by WHERE id = :id', { id, status: 'approved', audited_by: me.id, updated_by: me.id });
    if (!result || result.affectedRows === 0) {
        res.status(404).json({ success: false, error: 'NOT_FOUND' });
        return;
    }
...
router.post('/:id/reject', requirePermission('hscodes:approve'), asyncHandler(async (req, res) => {
    ...
    const id = req.params.id;
    const me = req.user;
    const [result] = await db.query('UPDATE hscodes SET status = :status, audited_by = :audited_by, audited_at = NOW(), rejection_reason = :rejection_reason, updated_by = :updated_by WHERE id = :id', {
        id,
        status: 'rejected',
        audited_by: me.id,
        updated_by: me.id,
