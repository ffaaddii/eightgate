...
router.get('/', asyncHandler(async (req, res) => {
    const [rows] = await db.query('SELECT id, username, display_name, role, enabled, created_at, updated_at FROM users ORDER BY created_at DESC LIMIT 500');
    res.status(200).json({
        success: true,
        data: rows.map((r) => ({
            id: String(r.id),
...
router.post('/', requirePermission('users:write'), asyncHandler(async (req, res) => {
    ...
    try {
        await db.query('INSERT INTO users (id, username, display_name, password_hash, role, enabled) VALUES (:id, :username, :display_name, :password_hash, :role, :enabled)', {
            id,
            username,
...
router.patch('/:id', requirePermission('users:write'), asyncHandler(async (req, res) => {
    ...
    const [result] = await db.query(`UPDATE users SET ${updates.join(', ')} WHERE id = :id`, params);
...
router.post('/:id/reset-password', requirePermission('users:write'), asyncHandler(async (req, res) => {
    ...
    const [result] = await db.query('UPDATE users SET password_hash = :password_hash WHERE id = :id', { id, password_hash });
    if (!result || result.affectedRows === 0) {
        res.status(404).json({ success: false, error: 'NOT_FOUND' });
        return;
    }
