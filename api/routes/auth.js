...
router.post('/login', asyncHandler(async (req, res) => {
    ...
    const [rows] = await db.query('SELECT id, username, display_name, password_hash, role, enabled FROM users WHERE username = :username LIMIT 1', { username });
    ...
    if (enforceDevices) {
        ...
        const [deviceRows] = await db.query('SELECT id, enabled FROM user_devices WHERE user_id = :user_id AND device_id = :device_id LIMIT 1', { user_id: String(userRow.id), device_id: deviceId });
        ...
        const [deviceRows] = await db.query('SELECT id FROM user_devices WHERE user_id = :user_id AND device_id = :device_id LIMIT 1', { user_id: String(userRow.id), device_id: deviceId });
        if (!deviceRows?.[0]) {
            await db.query('INSERT INTO user_devices (id, user_id, device_id, label, enabled, approved_by) VALUES (:id, :user_id, :device_id, :label, :enabled, :approved_by)', {
                id: randomUUID(),
    ...
    await writeAudit({
        req,
        user: { id: String(userRow.id), username: String(userRow.username), role: userRow.role },
