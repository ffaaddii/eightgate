...
async function main() {
    ...
    const [existing] = await conn.query('SELECT id FROM users WHERE username = ? LIMIT 1', [seedUsername]);
    if (existing.length === 0) {
        const id = randomUUID();
        const password_hash = await bcrypt.hash(seedPassword, 12);
        await conn.query('INSERT INTO users (id, username, display_name, password_hash, role, enabled) VALUES (?, ?, ?, ?, ?, ?)', [id, seedUsername, seedDisplayName, password_hash, 'superadmin', 1]);
    }
    await conn.end();
}
async function ensureHscodeColumns(conn, database) {
